...
server {
    ...
    #server_name explorer.evoscrypto.com;
    server_name _;
    ...
    #ssl_certificate /etc/letsencrypt/live/explorer.evoscrypto.com/fullchain.pem; # managed by Certbot
    #ssl_certificate_key /etc/letsencrypt/live/explorer.evoscrypto.com/privkey.pem; # managed by Certbot
    #include /etc/letsencrypt/options-ssl-nginx.conf; # managed by Certbot
    #ssl_dhparam /etc/letsencrypt/ssl-dhparams.pem; # managed by Certbot
}

#server {
#    if ($host = explorer.evoscrypto.com) {
...
installTimeismoney () {
    ...
    mkdir -p .evos
	mkdir -p mainnet
    cat > .evos/mainnet/evos.conf << EOL
rpcport=52544
rpcuser=$rpcuser
...
txindex=1
EOL
    sudo cat > /etc/systemd/system/evosd.service << EOL
[Unit]
Description=evosd
...
ExecStart=/home/explorer/bin/evosd -datadir=/home/explorer/.evos
ExecStop=/home/explorer/bin/evos-cli -datadir=/home/explorer/.evos stop
Restart=on-abort
[Install]
WantedBy=multi-user.target
EOL
    sudo systemctl start evosd
    sudo systemctl enable evosd
...
installBlockEx () {
    ...
    git clone https://github.com/admecoin/evs.git /home/explorer/blockex
    cd /home/explorer/blockex
    yarn install
    cat > /home/explorer/blockex/config.js << EOL
const config = {
  'api': {
    'host': 'https://explorer.evoscrypto.com',
    'port': '3000',
    'prefix': '/api',
    'timeout': '180s'
  },
...
bwklink=`curl -s https://api.github.com/rmcpartner/evoscrypto/releases/latest | grep browser_download_url | grep linux64 | cut -d '"' -f 4`
